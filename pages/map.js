  ...
  useEffect(() => {
    const initMap = async () => {
      try {
        ...
        console.log('📦 Loader 생성 완료, Google Maps 로딩 중...');
        const google = await loader.load();
        console.log('✅ Google Maps API 로딩 완료!');

        // DOM 요소 확인
        if (!mapRef.current) {
          console.error('❌ mapRef.current가 null입니다. DOM 요소를 찾을 수 없습니다.');
        ...
        createExampleMarker(examplePhotoSphere2, 'midle-east');
        ...
        // 초기 Photo Sphere 검색
        google.maps.event.addListenerOnce(map, 'tilesloaded', () => {
          console.log('🔍 초기 Photo Sphere 검색 시작...');
